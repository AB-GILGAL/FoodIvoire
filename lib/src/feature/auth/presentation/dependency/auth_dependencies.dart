...
injectAuthDependencies() {
  ...
  getIt.registerLazySingleton<RequestOTP>(
    () => RequestOTP(getIt<AuthRepoImpl>()),
  );
  getIt.registerLazySingleton<LoginCustomer>(
    () => LoginCustomer(getIt<AuthRepoImpl>()),
  );
  getIt.registerLazySingleton<CreateCustomer>(
    () => CreateCustomer(getIt<AuthRepoImpl>()),
  );
}

final authProvider = AuthProvider(
  requestOTP: getIt<RequestOTP>(),
  verifyOTP: getIt<LoginCustomer>(),
